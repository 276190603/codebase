...
xMBTCPPortInit( USHORT usTCPPort )
{
    ...
    xListenSocket   = socket(( int )AF_INET,
                             ( int )SOCK_STREAM,
                             ( int )IPPROTO_TCP);
    
    Mem_Set((void *)&serveraddr, (CPU_INT08U) 0,(CPU_SIZE_T)(sizeof (struct sockaddr_in)));
    serveraddr.sin_family = AF_INET;
    serveraddr.sin_addr.s_addr = htonl( INADDR_ANY );
    serveraddr.sin_port = htons( usPort );
    
    if( bind( ( int ) xListenSocket,( struct sockaddr *)&serveraddr, ( socklen_t )(sizeof (struct sockaddr_in))) != 0)
    {
        return FALSE;
    }
    else if( listen( ( int ) xListenSocket, (CPU_INT08U) 5 ) != 0 )
    {
        return FALSE;
    }
    FD_ZERO( &allset );
...
xMBPortTCPPool( void )
{
    ...
    while( TRUE )
    {
        ...
        if( ret > 0 )
        {
            ...
            {
                if( ( ( ret = recv( xClientSocket, &aucTCPBuf[usTCPBufPos], usTCPFrameBytesLeft, 0 ) )
                                  == SOCKET_ERROR ) || ( !ret ) )
                {
                    close( xClientSocket );
    ...
    do
    {
        ...
        case -1:
            if( iTimeOut > 0 )
            {
                ...
                OSTimeDlyHMSM(0, 0, 0, MB_TCP_READ_CYCLE);
            }
            else
            {
                bAbort = TRUE;
...
prvbMBPortAcceptClient(  )
{
    ...
    else if( ( xNewSocket = accept(xListenSocket, &their_addr, &sin_size) ) == INVALID_SOCKET )
    {
        bOkay = FALSE;
    }
    else
