...
#include "24cxx.h"
#include "includes.h"
volatile unsigned char USART2_REC_Flag = 0;
unsigned char USART2_buff[USART2_REC_BUFF_SIZE];//用于接收数据
...
unsigned char USART_RX_BUF2[30];
unsigned char USART_TX_BUF2[200];

unsigned short int setRegisterVal2(unsigned short int addr,unsigned short int tempData);
unsigned char localAddr2 = 1;

unsigned short int getRegisterVal2(unsigned short int addr,unsigned short int *tempData);
/******************USART2配置****485接口**********************/
...
void USART2_IRQHandler(void)  
{
  ...
  {	 
    ...
    if(USART2_rec_counter<64)
    {
      USART2_buff[USART2_rec_counter]=res;		//记录接收到的值
      USART2_rec_counter++;						//接收数据增加1 
    } 
  }  
}
...
void USART2_Send_Data(unsigned char *send_buff,unsigned int length)
{
  ...
  while(USART_GetFlagStatus(USART2, USART_FLAG_TC) == RESET);	 	
  USART2_rec_counter = 0;
  USART2_RX_EN;  		//485接收使能
}
...
{
  unsigned char rxlen=USART2_rec_counter;
  unsigned char i=0;
  *len=0;				//默认为0
  OSTimeDlyHMSM(0, 0, 0, 10);		//等待10ms,连续超过10ms没有接收到一个数据,则认为接收结束
  //delay_ms(10);		//等待10ms,连续超过10ms没有接收到一个数据,则认为接收结束
  if(rxlen==USART2_rec_counter&&rxlen)//接收到了数据,且接收完成了
  {
    for(i=0;i<rxlen;i++)
    {
      buf[i]=USART2_buff[i];	
    }		
    *len=USART2_rec_counter;	//记录本次数据长度
    USART2_rec_counter=0;		//清零
  }
}
unsigned char	sendCount_USART2;		//发送字节个数

/*==================================================================================*/
// 语法格式：void Send_Hex2(unsigned char c)
...
{	
  USART2_Send_Data(&c,1);
}
/*==================================================================================*/
// 语法格式：unsigned int crc16_USART2(unsigned char *puchMsg, unsigned int usDataLen) 
// 功能描述: 对数据进行CRC校验
...
/*==================================================================================*/
/* CRC 高位字节值表 */ 
const unsigned char auchCRCHi2[] = { 
...
/* CRC低位字节值表*/ 
const unsigned char auchCRCLo2[] = { 
...
unsigned short int crc16_USART2(unsigned char *puchMsg, unsigned short int usDataLen) 
{ 
  ...
  { 
    ...
    uchCRCHi = uchCRCLo ^ auchCRCHi2[uIndex] ; 
    uchCRCLo = auchCRCLo2[uIndex] ; 
  } 
...
void readRegisters2(void)
{
  ...
  addr = (USART_RX_BUF2[2]<<8) + USART_RX_BUF2[3];   //
  //tempAddr = addr & 0xfff;	
  
  tempAddr = addr;
  
  readCount = (USART_RX_BUF2[4]<<8) + USART_RX_BUF2[5];	//要读的个数
  //readCount = receBuf[5];
  ...
  {
    getRegisterVal2(tempAddr,&tempData);				
    USART_TX_BUF2[i+3] = tempData >> 8;				   		
    USART_TX_BUF2[i+4] = tempData & 0xff;			
  }
  
  USART_TX_BUF2[0] = localAddr2;
  USART_TX_BUF2[1] = 3;
  USART_TX_BUF2[2] = byteCount;
  byteCount += 3;
  crcData = crc16_USART2(USART_TX_BUF2,byteCount);
  USART_TX_BUF2[byteCount] = crcData >> 8;
  byteCount++;
  USART_TX_BUF2[byteCount] = crcData & 0xff;
  
  sendCount_USART2 = byteCount + 1;
  for(i=0;i<sendCount_USART2;i++)
  {
    Send_Hex2(USART_TX_BUF2[i]);
  }
}

void readRegisters04_USART2(void)
{
  unsigned short int addr;
  unsigned short int tempAddr;
  unsigned short int crcData;
...
void presetSingleRegister2()
{
  ...
  addr = (USART_RX_BUF2[2]<<8) | USART_RX_BUF2[3];
  tempAddr = addr & 0xfff;
  tempData = (USART_RX_BUF2[4]<<8) | USART_RX_BUF2[5];
  
  setRegisterVal2(tempAddr,tempData);			
  
  USART_TX_BUF2[0] = localAddr2;
  USART_TX_BUF2[1] = 6;
  USART_TX_BUF2[2] = addr >> 8;
  USART_TX_BUF2[3] = addr & 0xff;
  USART_TX_BUF2[4] = tempData>>8;
  USART_TX_BUF2[5] = tempData&0xff;
  crcData = crc16_USART2(USART_TX_BUF2,6);
  USART_TX_BUF2[6] = crcData >> 8;
  USART_TX_BUF2[7] = crcData & 0xff;
  sendCount_USART2 = 8;
  for(i=0;i<sendCount_USART2;i++)
  {
    Send_Hex2(USART_TX_BUF2[i]);
...
// 出口参数: 无
/*==================================================================================*/
void presetMultipleRegisters2(void)
{
  unsigned short int addr;
  unsigned short int tempAddr;
  unsigned short int byteCount;
  ...
  //	addr = USART_RX_BUF2[3];
  //	tempAddr = addr & 0xff;
  addr = (USART_RX_BUF2[2]<<8) + USART_RX_BUF2[3];
  tempAddr = addr;
  
  setCount = USART_RX_BUF2[5];
  byteCount = USART_RX_BUF2[6];	
  
  for(i=0;i<setCount;i++,tempAddr++)
  {
    tempData = (USART_RX_BUF2[i*2+7]<<8) + USART_RX_BUF2[i*2+8];
  ...
  USART_TX_BUF2[2] = addr >> 8;
  USART_TX_BUF2[3] = addr & 0xff;
  USART_TX_BUF2[4] = setCount >> 8;
  USART_TX_BUF2[5] = setCount & 0xff;
  crcData = crc16_USART2(USART_TX_BUF2,6);
  USART_TX_BUF2[6] = crcData >> 8;
  USART_TX_BUF2[7] = crcData & 0xff;
  sendCount_USART2 = 8;
...
void checkComm2Modbus(void)
{
  ...
  USART2_Receive_Data(USART_RX_BUF2,&len);
  if(len>7)
  {
    ...
    case 3:				      //读取保持寄存器(一个或多个)
      ...
      {						
        len=0;
        if(USART_RX_BUF2[0]==localAddr2)
        {
          crcData = crc16_USART2(USART_RX_BUF2,6);	
          if(crcData == USART_RX_BUF2[7]+(USART_RX_BUF2[6]<<8))
          {
            readRegisters2();
    ...
    case 16://设置多个寄存器
      if(USART_RX_BUF2[0]==localAddr2)
      {
        if(len>7)
        {
          ...
          if(len== tempData)
          {
            crcData = crc16_USART2(USART_RX_BUF2,len-2);
            if(crcData == (USART_RX_BUF2[len-2]<<8)+ USART_RX_BUF2[len-1])
            {
              presetMultipleRegisters2();
              len=0;
            }
          }
        }					
