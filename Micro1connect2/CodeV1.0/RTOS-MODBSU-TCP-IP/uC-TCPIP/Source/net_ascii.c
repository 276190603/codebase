        ...
        switch (*pchar_cur) {
            ...
            case 'f':
                 ...
                 switch (*pchar_cur) {                                  /* Convert ASCII char into hex val.             */
                     ...
                     case '8':
                     case '9':
                          addr_octet_val_dig = (CPU_INT08U)(*pchar_cur - '0');
                          break;
                     ...
                     case 'E':
                     case 'F':
                          addr_octet_val_dig = (CPU_INT08U)(*pchar_cur - 'A' + 10);
                          break;
...
/*$PAGE*/
            case '-':
            case ':':
                 if (pchar_prev == (CPU_CHAR *)0) {                     /* If NO prev char  (see Note #1b2B1); ...      */
                     Mem_Clr((void     *)paddr_mac,                     /* ... clr rtn addr (see Note #2c)     ...      */
                             (CPU_SIZE_T)NET_ASCII_NBR_OCTET_ADDR_MAC);
                 ...
                 if ((*pchar_prev == (CPU_CHAR)'-') ||                  /* If prev char a hyphen                   ...  */
                     (*pchar_prev == (CPU_CHAR)':')) {                  /* ... or       a colon (see Note #1b2B1); ...  */
                     Mem_Clr((void     *)paddr_mac,                     /* ... clr rtn addr     (see Note #2c)     ...  */
                             (CPU_SIZE_T)NET_ASCII_NBR_OCTET_ADDR_MAC);
                    *perr = NET_ASCII_ERR_INVALID_CHAR_SEQ;             /* ... & rtn err.                               */
                     return;
    ...
    if (pchar_cur == (CPU_CHAR *)0) {                                   /* If NULL ptr in ASCII str (see Note #1b1); .. */
        Mem_Clr((void     *)paddr_mac,                                  /* .. clr rtn addr          (see Note #2c)   .. */
                (CPU_SIZE_T)NET_ASCII_NBR_OCTET_ADDR_MAC);
    ...
    if ((*pchar_prev == (CPU_CHAR)'-') ||                               /* If last char a hyphen                   ..   */
        (*pchar_prev == (CPU_CHAR)':')) {                               /* .. or        a colon (see Note #1b2B1); ..   */
        Mem_Clr((void     *)paddr_mac,                                  /* .. clr rtn addr      (see Note #2c)     ..   */
                (CPU_SIZE_T)NET_ASCII_NBR_OCTET_ADDR_MAC);
       *perr = NET_ASCII_ERR_INVALID_CHAR_SEQ;                          /* .. & rtn err.                                */
                           ...
                           NET_ERR      *perr)
{
   ...
   *pchar = (CPU_CHAR)0;                                                /* Append NULL char (see Note #1b1C).           */
                                 ...
                                 NET_ERR   *perr)
{
    ...
    CPU_CHAR     *pchar_prev;
    NET_IP_ADDR   addr_ip;
    CPU_INT16U    addr_octet_val;                                       /* See Note #2.                                 */
    CPU_INT08U    addr_nbr_octet;
    CPU_INT08U    addr_nbr_octet_dig;
    ...
    while ((pchar_cur != (CPU_CHAR *)0) &&                              /* Parse ALL non-NULL chars in ASCII str.       */
        ...
        switch (*pchar_cur) {
            ...
            case '.':
                 ...
                 if (*pchar_prev == (CPU_CHAR)'.') {                    /* If prev char a dot  (see Note #1b2B1), ...   */
                     *perr =  NET_ASCII_ERR_INVALID_CHAR_SEQ;           /* ... rtn err.                                 */
                      return (NET_IP_ADDR_NONE);
                 }
                 ...
                 addr_ip             += (NET_IP_ADDR)addr_octet_val;
                 addr_octet_val       =  0x0000;
                 addr_nbr_octet_dig   =  0;
                 break;
...
/*$PAGE*/
    ...
    if (*pchar_prev == (CPU_CHAR)'.') {                                 /* If last char a dot (see Note #1b2B1), ..     */
        *perr =  NET_ASCII_ERR_INVALID_CHAR_SEQ;                        /* .. rtn err.                                  */
         return (NET_IP_ADDR_NONE);
    }
