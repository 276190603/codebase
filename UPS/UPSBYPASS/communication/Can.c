...
Uint16 CanSendDataWithPtc(Uint16 FunCode,float data1,float data2,Uint16 data3,MailBoxType mbType) ;   //采用通讯协议发送数据
...
void UseCan(void)
{
	...
	CAN.SendId.Id = 9;								//发送邮箱ID为9与触屏通讯邮箱
	CAN.RecId.Id = 4;								//接收邮箱ID为4与逆变板通讯邮箱
	CAN.RecIdMask.Id = 0x0000000f;						//接收帧ID第0,1位必须相同
	CAN.AutoSendId.Id = 5;							//自动发送邮箱ID为5与逆变板通讯邮箱
	CAN.AutoRecId.Id = 9;							//自动接收邮箱ID为9与触屏通讯邮箱
	...
	CAN.SendDataWithPtc = CanSendDataWithPtc2;
 ...
 Uint16 CanSendDataWithPtc(Uint16 FunCode,float data1,float data2,Uint16 data3,MailBoxType mbType)    //采用通讯协议发送数据
{
    ...
    reVal = CanSendData(3,mbType);      //发送数据
    return reVal;
}
...
Uint16 CanScan(void)		//扫描有无中断需要处理
{
	...
	{
		if(CAN.RecNum == 1)		//接收到五个字节
		{
			switch(CAN.RecBuf[0])
			{
                case 0:        //逆变输出A相电压
                    if(myScr.SOFTSTART==0)
                    {
                        CAN.SendDataWithPtc(0,AutoSendMailBox);   //开逆变关旁路
                        //if(myHardwarePool.bit.CpuTimer1 == 0)       //Cpu定时器1正在被使用
                       // {
                        //  myHardwarePool.bit.CpuTimer1 = 1;       //Cpu定时器1正在被使用
                          ConfigCpuTimer(&CpuTimer1, 150, 800000);    //Cpu定时器1的周期为400ms 关旁路
                          IER |= M_INT13;         /*定时器1开中断*/
                ...
                case 1:
                    if(myScr.SOFTSTART==0)
                    {
                        //ConfigCpuTimer(&CpuTimer1, 150, 20000);    //Cpu定时器1的周期为400ms 关旁路
                        //IER |= M_INT13;         /*定时器1开中断*/
                        //StartCpuTimer1(); //启动Cpu定时器0
                        ...
                        else
                        {
                           ...
                           // {
                             // myHardwarePool.bit.CpuTimer1 = 1;       //Cpu定时器1正在被使用
                              ConfigCpuTimer(&CpuTimer1, 150, 800000);    //Cpu定时器1的周期为400ms 开旁路
                              IER |= M_INT13;         //定时器1开中断
                              StartCpuTimer1(); //启动Cpu定时器0
                            //}
                        }  //发送数据}
	    ...
	    if(CAN.AutoRecNum  == 5)     //接收到五个字节
	    {
            switch(CAN.AutoRecBuf[0])
            {
              //  case 0:        //逆变输出A相电压
                   ...
                   // break;
                case 1:        //逆变输出A相电压
                    CanSendDataWithPtc(FRA_BP_ADD,myScr.UA_BPRMS,myScr.FrA_BP,SOFT_START,SendMailBox);   //发送数据
                    break;
                default:
                    return 2;
            }
