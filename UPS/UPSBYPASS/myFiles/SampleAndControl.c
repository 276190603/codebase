...
Uint16 I_KiEnable = 0;
Uint16 Ki_Enable = 0;
unsigned int SOFT_START=0;
unsigned int FIRST_START=2;
float PID=180;
 ...
 void UseControlSystem(void)
{
	...
	myControlSystem.SoftStart.SS_Time = 3000;	//软启动用时：1000 ms
...
//	myControlSystem.Sample.B_Sample[11] = -444.0385;		//B4截距	()
	...
	//myControlSystem.Sample.K_Sample[11] = 0.0135;       //B4斜率
  //  myControlSystem.Sample.B_Sample[11] = -28.3;        //B4截距  ()

    myControlSystem.Sample.K_Sample[11] = 0.2134;       //B4斜率
    myControlSystem.Sample.B_Sample[11] = -408.2;        //B4截距  ()

	myControlSystem.Sample.K_Sample[12] = 0.2462;				//B3斜率(UC_SCR)
	myControlSystem.Sample.B_Sample[12] = -514.6332;				//B3截距

 //   myControlSystem.Sample.K_Sample[13] = 0.3596 ;    //B5斜率(UA_SCR)
 //   myControlSystem.Sample.B_Sample[13] = -616;      //B5截距
...
*/
			...
			myScr.UA_SCROutRMS = sqrt(temp1)-3;				//整流输出A相电压有效值
			sumUASCROut = 0;
/*
...
*/
			...
			myScr.UA_BPRMS = sqrt(temp1)-3;
			//myScr.UA_BPRMS = sqrt(temp1)-8;
			sumUABP = 0;
...
//ConfigCpuTimer(&CpuTimer1, 150, 20000);    //Cpu定时器1的周期为20ms
///IER |= M_INT13;         /*定时器1开中断*/
...
interrupt void CpuTimer1_ISR(void)
{
    ...
    switch(CAN.RecBuf[0])
    {
        case 0:
            ...
            //DELAY_US(200);
            StopCpuTimer1();
           // myHardwarePool.bit.CpuTimer1 = 0;       //Cpu定时器1正在被使用
        break;
        case 1:
            /*
            scr_number_switch(scr_1,1);
            ...
            //DELAY_US(200);
            StopCpuTimer1();
            CAN.SendDataWithPtc(1,AutoSendMailBox);   //发送数据
            */
