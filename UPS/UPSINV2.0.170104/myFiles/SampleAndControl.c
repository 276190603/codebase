...
float C_OUTPUT1 = 0;
int16 RemainStep1 = 0;   //软启动剩余步数
float OutPerStep1 = 0;   //软启动输出步长

float C_OUTPUT2 = 0;
int16 RemainStep2 = 0;   //软启动剩余步数
float OutPerStep2 = 0;   //软启动输出步长
//带遗忘因子的有效值计算
 ...
 void UseControlSystem(void)
{
	...
	myControlSystem.Controller.Output = 0.05;	//输出调制度：0.7
	myControlSystem.Controller.State = Disable;	//禁止控制器
	myControlSystem.Controller.Target = 220;	//目标值：220

	//软启动模块
	myControlSystem.SoftStart.SS_Stairs = 10;	//分4个阶段启动
	myControlSystem.SoftStart.SS_Target = 0.7;	//启动后控制器输出：0.7
	myControlSystem.SoftStart.SS_Time = 400;	//软启动用时：1000 ms
	...
	myControlSystem.Sample.B_Sample[9] = -444.0385;			//B1截距(UC_Inv)

	myControlSystem.Sample.K_Sample[10] = 0.2133;		//B2斜率
	myControlSystem.Sample.B_Sample[10] = -444.0385;		//B2截距(UB_Inv)

	myControlSystem.Sample.K_Sample[11] = 0.2133;		//B4斜率
	myControlSystem.Sample.B_Sample[11] = -444.0385;		//B4截距	(UA_Inv)

	myControlSystem.Sample.K_Sample[12] = 0.2462;				//B3斜率(BP_C)
	myControlSystem.Sample.B_Sample[12] = -514.6332;				//B3截距

	myControlSystem.Sample.K_Sample[13] = 0.2462;		//B5斜率(BP_A)
	myControlSystem.Sample.B_Sample[13] = -514.6332;		//B5截距

	myControlSystem.Sample.K_Sample[14] = 0.2462;		//B6斜率(BP_B)
...
Uint16 IsAddControl(void)				//判断是否可以投入控制器，并改变其状态
{
	...
	if((myInverter.U_DCRMS >= 176)&(myInverter.U_DCRMS <= 264))	//输出值在窗口内开始调节 176 264
...
Uint16 Control(void)						//控制过程
{
	...
	myControlSystem.Controller.InsUA_Out = CalRMS(myInverter.UA_InvOut,UaFifo,&UaPtr,&UaSa,&UaPh);	//计算瞬时有效值
...
Uint16 SACInit(void)						//采样控制初始化
{
	...
	myControlSystem.Sample.SampleRatio = SAMPLE_FRE/myInverter.FrOut;	//计算采样比
...
//-----------------------------------------------------------------------------
// 输入参数：	state -> 采样模块状态
// 输出参数： 	0 -> 配置成功,1 -> 输入参数错误，2 -> 逆变器未使能,3 -> 硬件资源已被使用
//-----------------------------------------------------------------------------
...
Uint16 SAC_Scan(void)						//用于在主程序循环扫描，在一个采样周期内该函数必须执行至少一次，否则会丢失采样结果
{
	...
	if(myControlSystem.Sample.FlagRMS)		//使能有效值计算
	{
		...
		else
		{
			...
			myInverter.IA_InvOutRMS = sqrt(temp1);				//逆变输出电流A相有效值
			sumIAInvOut = 0;

			temp1 = sumIBInvOut/SampleN;						//sum^2/n
			myInverter.IB_InvOutRMS = sqrt(temp1);				//逆变输出电流B相有效值
			sumIBInvOut = 0;

			temp1 = sumICInvOut/SampleN;						//sum^2/n
			myInverter.IC_InvOutRMS = sqrt(temp1);				//逆变输出电流C相有效值
			sumICInvOut = 0;

			temp1 = sumIATotalOut/SampleN;
			myInverter.IA_TotalOutRMS = sqrt(temp1);			//逆变总输出电流A相有效值
			sumIATotalOut = 0;

			temp1 = sumIBTotalOut/SampleN;
			myInverter.IB_TotalOutRMS = sqrt(temp1);			//逆变总输出电流B相有效值
			sumIBTotalOut = 0;

			temp1 = sumICTotalOut/SampleN;
			myInverter.IC_TotalOutRMS = sqrt(temp1);			//逆变总输出电流C相有效值
			sumICTotalOut = 0;

			temp1 = sumUDC/SampleN;
			myInverter.U_DCRMS = sqrt(temp1);					//DC母线电压有效值
			sumUDC = 0;

			temp1 = sumUAInvOut/SampleN;
			myInverter.UA_InvOutRMS = sqrt(temp1);				//逆变输出A相电压有效值
			sumUAInvOut = 0;

			temp1 = sumUBInvOut/SampleN;
			myInverter.UB_InvOutRMS = sqrt(temp1);				//逆变输出B相电压有效值
			sumUBInvOut = 0;

			temp1 = sumUCInvOut/SampleN;
			myInverter.UC_InvOutRMS = sqrt(temp1);				//逆变输出C相电压有效值
			sumUCInvOut = 0;

			temp1 = sumUABP/SampleN;
			myInverter.UA_BPRMS = sqrt(temp1);					//旁路A相电压有效值
			sumUABP = 0;

			temp1 = sumUBBP/SampleN;
			myInverter.UB_BPRMS = sqrt(temp1);					//旁路B相电压有效值
			sumUBBP = 0;

			temp1 = sumUCBP/SampleN;
			myInverter.UC_BPRMS = sqrt(temp1);					//旁路C相电压有效值
			sumUCBP = 0;
...
Uint16 StartRun(void)                   //软启动
{
    float deltaUout1 = 0.7 - C_OUTPUT1;
    if((deltaUout1 > -0.1)&(deltaUout1 < 0.1))        //当前值和目标值距离过近不用软起
        return 2;
    if(myHardwarePool.bit.CpuTimer1)        //Cpu定时器1已被使用
        return 1;
    myHardwarePool.bit.CpuTimer1 = 1;       //Cpu定时器1正在被使用
 ...
 //   Uint16 _TperStep1 = myControlSystem.SoftStart.SS_Time/myControlSystem.SoftStart.SS_Stairs;   //ms
    ...
    RemainStep1 = 10;       //时间步长
    OutPerStep1 = deltaUout1/10; //输出步长
    XIntruptRegs.XNMICR.bit.SELECT = 0;         //Timer1连到int13

    IER |= M_INT13;         /*定时器1开中断*/
    StartCpuTimer1();       //启动定时器1

    while(RemainStep1 > 0)               //等待软启动完成
    {
        myControlSystem.Sample.Scan();  //软起等待过程中会进行采样
        myPhaseLock.Scan();  //允许进行锁相
...
Uint16 StartRun2(void)                   //软启动
{
    float deltaUout2 = 0.7 - C_OUTPUT2;
    if((deltaUout2 > -0.1)&(deltaUout2 < 0.1))        //当前值和目标值距离过近不用软起
        return 2;
    if(myHardwarePool.bit.CpuTimer1)        //Cpu定时器1已被使用
        return 1;
    myHardwarePool.bit.CpuTimer1 = 1;       //Cpu定时器1正在被使用
 ...
 //   Uint16 _TperStep1 = myControlSystem.SoftStart.SS_Time/myControlSystem.SoftStart.SS_Stairs;   //ms
    ConfigCpuTimer(&CpuTimer1,150,300*1000.0);
    RemainStep2 = 10;       //时间步长
    OutPerStep2 = deltaUout2/10; //输出步长
    XIntruptRegs.XNMICR.bit.SELECT = 0;         //Timer1连到int13

    IER |= M_INT13;         /*定时器1开中断*/
    StartCpuTimer1();       //启动定时器1

    while(RemainStep2 > 0)               //等待软启动完成
    {
        myControlSystem.Sample.Scan();  //软起等待过程中会进行采样
        myPhaseLock.Scan();  //允许进行锁相
    }
...
Uint16 ActuatorInit(void)					//执行机构初始化
{
	...
	myControlSystem.Actuator.CarRatio = 1000*myControlSystem.Actuator.CarFre/myInverter.FrOut;	//载波比
...
//-----------------------------------------------------------------------------
// 输入参数：	state -> 执行机构状态
// 输出参数： 	0 -> 配置成功,1 -> 输入参数错误，2 -> 逆变器未使能，4 -> EPW已被占用
//-----------------------------------------------------------------------------
...
interrupt void Epwm1_ISR(void)
{
	...
	if(CurPhase <= PI)		//前半周期
	{
		...
		EPwm1Regs.CMPA.half.CMPA = (Uint16)(C_OUTPUT*EPwm1Regs.TBPRD*sin(CurPhase));		//控制器输出×周期值×sin(CurPhase)
		//EPwm2Regs.CMPA.half.CMPA = (Uint16)(COPRD);       //控制器输出×周期值×sin(CurPhase)
	}
	else					//后半周期
	{
		EPwm1Regs.CMPA.half.CMPA = 0;		//V3通,V4断
		EPwm2Regs.CMPA.half.CMPA = (Uint16)(C_OUTPUT*EPwm1Regs.TBPRD*sin(CurPhase-PI));	//控制器输出×周期值×sin(CurPhase)
