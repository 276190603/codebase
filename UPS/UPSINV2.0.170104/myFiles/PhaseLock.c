...
Uint16 PhaseLockScan(void)
{
	...
	myInverter.FrA_BP = 3000000/((ValCap+1)*1.0)*50;						//计算频率(50Hz对应3000000)
   myInverter.FrA_INV = 3000000/((ValCap5+1)*1.0)*50;                        //计算频率(50Hz对应3000000)

	if(myPhaseLock.StatePL == Enable)	//使能锁相同步
	{
		Uint32 _180P = ValCap/2;			//360/2=180°对应计数值
		Uint32 _2_8P = ValCap>>8;			//360/2^7 = 2.8125°对应计数值
		Uint32 _5_8P = ValCap>>6;           //360/2^7 = 2.8125°对应计数值
		...
		if((myPhaseLock.ZeroTimeAct >(ValCap-_5_8P))&&(myPhaseLock.ZeroTimeAct < ValCap))
		    {myPhaseLock.PSD_BYP = NoShift;}
		else if((myPhaseLock.ZeroTimeAct >0)&&(myPhaseLock.ZeroTimeAct < _5_8P))
		{ myPhaseLock.PSD_BYP = NoShift;}
		else
		    {myPhaseLock.PSD_BYP = NotShift;}
	}
	else
	{
		myPhaseLock.PSD = NoneShift;						//不移动
	    myPhaseLock.PSD_BYP = NotShift;
	}
