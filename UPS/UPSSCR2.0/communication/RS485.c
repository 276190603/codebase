...
Uint16 RS485Scan(void)					//485扫描函数
{
	...
	if(VerifyFrame(RS485Mdl) == 0)		//验证地址、功能码、读取寄存器个数、校验是否正确
	{
		...
		{
		   case SYS_START:
		       myScr.SOFTSTART = (RS485.RecBuffer[7]<<8)|(RS485.RecBuffer[8]);      //读取数据的地址
		       RS485MB16Return();     //以modbus 03 命令返回形式返回数据
		    break;
			case UA_SCROUT_ADD:			//A相输出电压
				RS485MB03Return(myScr.UA_BPRMS);		//以modbus 03 命令返回形式返回数据
				break;
            case UB_SCROUT_ADD:         //B相输出电压
                RS485MB03Return(myScr.UB_BPRMS);        //以modbus 03 命令返回形式返回数据
                break;
            case UC_SCROUT_ADD:         //C相输出电压
                RS485MB03Return(myScr.UC_BPRMS);        //以modbus 03 命令返回形式返回数据
                break;
            case FRAB_PHASE_ADD:         //AB相角
                RS485MB03Return(myScr.Phase_AB);        //以modbus 03 命令返回形式返回数据
                 break;
            case FRA_BP_ADD:         //A相输出频率
                RS485MB03Return(myScr.FrA_SCR);        //以modbus 03 命令返回形式返回数据
                break;
            case FRB_BP_ADD:         //B相输出频率
                RS485MB03Return(myScr.FrB_SCR);        //以modbus 03 命令返回形式返回数据
                break;
			case UA_SCROUTR_ADD:		//逆变A相输出电压有效值
				RS485MB03Return(myScr.U_DCRMS);		//以modbus 03 命令返回形式返回数据
				break;
			case DC_Target_ADD:
			    DC_Target = (unsigned int)(RS485.RecBuffer[7]<<8)|(RS485.RecBuffer[8]);      //读取数据的地址
	            RS485MB16Return();     //以modbus 03 命令返回形式返回数据
			    break;
			default:
				returnVal = 3;
