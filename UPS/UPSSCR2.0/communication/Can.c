...
void UseCan(void)
{
	...
	CAN.SendId.Id = 2;								//发送邮箱ID为0
	CAN.RecId.Id = 2;								//接收邮箱ID为1
	CAN.RecIdMask.Id = 0x0000000f;						//接收帧ID第0,1位必须相同
	CAN.AutoSendId.Id = 3;							//自动发送邮箱ID为2
	CAN.AutoRecId.Id = 3;							//自动接收邮箱ID为3
	...
	CAN.SendDataWithPtc = CanSendDataWithPtc;
...
Uint16 CanInit(void)
{
  ...
  // GpioCtrlRegs.GPAPUD.bit.GPIO17 = 0;   // Enable pull-up for GPIO21 (CANRXB)
   //  GpioCtrlRegs.GPAMUX2.bit.GPIO16 = 2;  // Configure GPIO20 for CANTXB operation
    ...
    ECanbMboxes.MBOX1.MSGID.bit.AME = 0;				//使用屏蔽掩码
    ECanbMboxes.MBOX1.MSGID.all |= CAN.RecId.Id;		//写入id

    ECanaLAMRegs.LAM1.all = 0;
    ECanaLAMRegs.LAM1.bit.LAMI = 1;						//使用屏蔽掩码
    ...
    ECanbMboxes.MBOX2.MSGID.all = 0;
    ECanbMboxes.MBOX2.MSGID.bit.IDE = 1;				//扩展标识符
    ECanbMboxes.MBOX2.MSGID.bit.AAM = 1;				//自动响应远程帧
    ECanbMboxes.MBOX2.MSGID.all |= CAN.AutoSendId.Id;	//写入id
...
*/
    ...
    ECanbMboxes.MBOX3.MSGID.all |= CAN.AutoSendId.Id;	//写入id
...
// 输出参数： 	0 -> 发送成功,1 -> CAN模块未使能,2 -> 输入参数错误
//-----------------------------------------------------------------------------
Uint16 CanSendDataWithPtc2(Uint16 FunCode,float data,MailBoxType mbType)	//采用通讯协议发送数据
{
	Uint16 reVal = 0;
	//Write2Buf(data,CanMdl);				//按协议将数据放入缓冲区中
	CAN.SendBuf[0] = data/256;			//填写功能码
	CAN.SendBuf[1] = data;           //填写功能码
...
Uint16 CanScan(void)		//扫描有无中断需要处理
{
	...
	if(CAN.Flag.bit.RecFrame)		//接收缓冲区有数据
	{
		if(CAN.RecNum == 5)		//接收到五个字节
		{
			switch(CAN.RecBuf[0])
			{
			//	case UA_SCROUT_ADD:		//逆变输出A相电压
			//		CanSendDataWithPtc(UA_SCROUT_ADD,myScr.UA_SCROut,SendMailBox);	//发送数据
				//	break;

		        case UB_SCROUT_ADD:        //逆变输出A相电压
                    CanSendDataWithPtc(UB_SCROUT_ADD,myScr.UA_BPRMS,myScr.UB_BPRMS,myScr.UC_BPRMS,myScr.FrA_SCR,myScr.FrB_SCR,myScr.SOFTSTART,SendMailBox);   //发送数据
                    break;
                case UA_SCROUTR_ADD:        //逆变输出A相电压
                    CanSendDataWithPtc2(UA_SCROUTR_ADD,myScr.U_DCRMS,AutoSendMailBox);   //发送数据
                    break;
                case SYS_START:        //逆变输出A相电压
                    myScr.SOFTSTART = CAN.RecBuf[1];
                    break;
                case DC_Target_ADD:        //逆变输出A相电压
                    DC_Target = (unsigned int)(CAN.RecBuf[1]<<8)|(CAN.RecBuf[2]);
                    break;
				default:
					return 2;
			}
